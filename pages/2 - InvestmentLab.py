...
with st.sidebar:
    ...
    st.markdown(
        "These KPIs, Analyses impact living standards within Ledgr's smaller Market in hand with their Global Counterparts.""")
    st.markdown("""Ledgr imbues a user's Investment efficacy via tactical information,leading one to various optimal scenarios (allocations, in other words""")
...
def figs(df, returns, mu, S):
    ...
    figRet.update_layout(
        title="Historic Returns at Daily Intervals", title_x=0.35)
    figMU = px.bar(df_mu, color=df_mu.index, text_auto='.2M')
    figMU.update_traces(textfont_size=14, textangle=0,
